      ...
      <div class="add-form" @submit.prevent="checkForm">
        ...
        <form>
          ...
          <div class="row-group-multi">
            ...
            <div class="form-group">
              <label for="">Количество вопросов <span>*</span></label>
              <input type="number"
                     class="row-group-control"
                     v-model="form.number_of_questions"
                     :class="{error: this.$v.form.number_of_questions.$dirty && !this.$v.form.number_of_questions.required}"
              >
            </div>
          </div>
          <div class="row-group">
    ...
    async add(){
      await this.$axios.post('/super-admin/create-test/', {
        ...
        .then(async (response) => {
          ...
          this.$router.push({name: 'teacher-home-work'})
        })
        .catch( async (error) => {
