              ...
              <div class="test-variant-single-body">
                <div class="variantLessons">
                  <div class="lesson" v-for="lesson in variantLessons" :id="lesson.id">
                    ...
                    <div class="lesson-add-question" v-if="!test.is_active && lesson.lesson_permission" @click="addQuestion(lesson.id)">
...
export default {
  ...
  methods:{
    ...
    resultTest(id){
      this.$router.push({name: 'teacher-by-lesson-result-id', params:{id: id}})
    },
    addQuestion(variant){
      this.$router.push({name: 'teacher-by-lesson-questions-variant', params:{variant: variant}})
    },
    addTest(){
      this.$router.push({name: 'teacher-by-lesson-add'})
    },
    ...
    async getVariantLessons() {
      try {
        const data =  (await this.$axios.get(`/teacher/variant-lessons/${this.variantSingle}/`)).data
    ...
    async getTestList() {
      try {
        const data =  (await this.$axios.get('/teacher/test-list/by_lesson/', {params: this.filter})).data
        this.testList = data
      }catch (er) {
        console.log(er.response)
      }
