<template>
  <div class="page">
    <div class="page-body">
<!--      <path-main />-->
      <nuxt-link class="add-button question-add" v-if="isCompleted" :to="{path: '/teacher/by-lesson/questions/'+variantID+'/add'}">
        Добавить вопрос
      </nuxt-link>
      <div class="questions">
        <div class="question-item" v-for="(question, i) in questions" :key="question.id">
          ...
          <div class="question-item-actions">
            <nuxt-link :to="{path: '/teacher/by-lesson/questions/'+variantID+'/edit/' + question.id}" class="edit">Изменить</nuxt-link>
...
export default {
  ...
  methods:{
    ...
    async getQuestions() {
      try {
        const data =  (await this.$axios.get(`/teacher/question-list/${this.variantID}/`)).data
        this.questions = data
        this.reRender();
      }catch (er) {
        console.log(er)
