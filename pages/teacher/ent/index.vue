...
export default {
  ...
  methods:{
    resultTest(id){
      this.$router.push({name: 'teacher-ent-result-id', params:{id: id}})
    },
    addQuestion(variant){
      this.$router.push({name: 'teacher-ent-questions-variant', params:{variant: variant}})
    },
    ...
    async getTestList() {
      try {
        const data =  (await this.$axios.get('/teacher/test-list/ent/', {params: this.filter})).data
        this.testList = data
      }catch (er) {
        console.log(er.response)
      }
