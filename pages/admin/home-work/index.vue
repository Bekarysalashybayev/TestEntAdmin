...
export default {
  ...
  methods:{
    ...
    addQuestion(variant){
      this.$router.push({name: 'admin-home-work-questions-variant', params:{variant: variant}})
    },
    ...
    async getTestList() {
      try {
        const data =  (await this.$axios.get('/super-admin/test-list/home_work/', {params: this.filter})).data
        this.testList = data
      }catch (er) {
        console.log(er.response)
      }
