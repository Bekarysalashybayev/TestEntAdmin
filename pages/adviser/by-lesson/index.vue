...
export default {
  ...
  methods:{
    resultTest(id){
      this.$router.push({name: 'adviser-by-lesson-result-id', params:{id: id}})
    },
    addQuestion(variant){
      console.log(variant)
      this.$router.push({name: 'adviser-by-lesson-questions-variant', params:{variant: variant}})
    },
    ...
    async getTestList() {
      try {
        const data =  (await this.$axios.get('/adviser/test-list/by_lesson/', {params: this.filter})).data
        this.testList = data
      }catch (er) {
        console.log(er.response)
      }
